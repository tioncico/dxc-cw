...
class GameDailyTaskPointRewardModel extends BaseModel
{
    ...
    public function userReceiveInfoToday($userId = -1)
    {
        if ($userId <= 0) {
            return null;
        }
    ...
    public function userReceiveInfoWeek($userId = -1)
    {
        if ($userId <= 0) {
            return null;
        }
        return $this->hasOne(UserDailyTaskReceiveModel::class, function (QueryBuilder $query) use ($userId) {
            $query->where('userId', $userId);
            $query->where('date', date('Ymd', mktime(0, 0, 0, date('m'), date('d') - date('w'), date('Y'))), '>=');
            return $query;
        }, 'rewardId', 'rewardId');
    }
}
