...
use EasySwoole\Oss\AliYun\Model\Tag;
use EasySwoole\Oss\AliYun\Model\DeleteObjectInfo;
...
class OssClientObjectVersioningTest extends TestOssClientBase
{
    public function testObjectBasic()
    {
        ...
        $ret1 = $this->ossClient->putObject($this->bucket, $object, $content1, array(OssConst::OSS_HEADERS => array('x-oss-object-acl' => 'public-read', 'x-oss-tagging' => 'key1=value1')))['headers'];
        $ret2 = $this->ossClient->putObject($this->bucket, $object, $content2, array(OssConst::OSS_HEADERS => array('x-oss-object-acl' => 'private', 'x-oss-tagging' => 'key2=value2')))['headers'];

        $this->assertTrue(isset($ret1[OssConst::OSS_HEADER_VERSION_ID]));
        $this->assertTrue(isset($ret2[OssConst::OSS_HEADER_VERSION_ID]));

        $versionId1 = $ret1[OssConst::OSS_HEADER_VERSION_ID];
        $versionId2 = $ret2[OssConst::OSS_HEADER_VERSION_ID];
        //get object
        $res = $this->ossClient->getObject($this->bucket, $object);
        $res1 = $this->ossClient->getObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        $res2 = $this->ossClient->getObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId2));
        $this->assertEquals($content1, $res1);
        $this->assertEquals($content2, $res2);
        $this->assertEquals($content2, $res);

        //meta
        $headers = $this->ossClient->getObjectMeta($this->bucket, $object) ['headers'];
        $headers1 = $this->ossClient->getObjectMeta($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1))['headers'];
        $headers2 = $this->ossClient->getObjectMeta($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId2))['headers'];
        $this->assertTrue(isset($headers[OssConst::OSS_HEADER_VERSION_ID]));
        $this->assertTrue(isset($headers1[OssConst::OSS_HEADER_VERSION_ID]));
        $this->assertTrue(isset($headers2[OssConst::OSS_HEADER_VERSION_ID]));
        $this->assertEquals($versionId1, $headers1[OssConst::OSS_HEADER_VERSION_ID]);
        $this->assertEquals($versionId2, $headers2[OssConst::OSS_HEADER_VERSION_ID]);
        $this->assertEquals($versionId2, $headers[OssConst::OSS_HEADER_VERSION_ID]);


        $sheaders = $this->ossClient->getSimplifiedObjectMeta($this->bucket, $object)['headers'];
        $sheaders1 = $this->ossClient->getSimplifiedObjectMeta($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1))['headers'];
        $sheaders2 = $this->ossClient->getSimplifiedObjectMeta($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId2))['headers'];

        $this->assertTrue(isset($sheaders[OssConst::OSS_HEADER_VERSION_ID]));
        $this->assertTrue(isset($sheaders1[OssConst::OSS_HEADER_VERSION_ID]));
        $this->assertTrue(isset($sheaders2[OssConst::OSS_HEADER_VERSION_ID]));
        $this->assertEquals($versionId1, $sheaders1[OssConst::OSS_HEADER_VERSION_ID]);
        $this->assertEquals($versionId2, $sheaders2[OssConst::OSS_HEADER_VERSION_ID]);
        $this->assertEquals($versionId2, $sheaders[OssConst::OSS_HEADER_VERSION_ID]);
        //acl
        $acl = $this->ossClient->getObjectAcl($this->bucket, $object);
        $acl1 = $this->ossClient->getObjectAcl($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        $acl2 = $this->ossClient->getObjectAcl($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId2));

        $this->assertEquals('public-read', $acl1);
        $this->assertEquals('private', $acl2);
        $this->assertEquals('private', $acl);

        $this->ossClient->putObjectAcl($this->bucket, $object, 'public-read-write', array(OssConst::OSS_VERSION_ID => $versionId1));
        $acl = $this->ossClient->getObjectAcl($this->bucket, $object);
        $acl1 = $this->ossClient->getObjectAcl($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        ...
        $tag1 = $this->ossClient->getObjectTagging($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        $tag2 = $this->ossClient->getObjectTagging($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId2));
        ...
        $this->ossClient->putObjectTagging($this->bucket, $object, $config, array(OssConst::OSS_VERSION_ID => $versionId1));
        $tag = $this->ossClient->getObjectTagging($this->bucket, $object);
        $tag1 = $this->ossClient->getObjectTagging($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        $this->assertEquals(1, count($tag1->getTags()));
        ...
        $this->ossClient->deleteObjectTagging($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        $tag = $this->ossClient->getObjectTagging($this->bucket, $object);
        $tag1 = $this->ossClient->getObjectTagging($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        ...
        $this->assertTrue($this->ossClient->doesObjectExist($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1)));
        $this->assertTrue($this->ossClient->doesObjectExist($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId2)));

        $dret1 = $this->ossClient->deleteObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        ...
        $this->assertFalse($this->ossClient->doesObjectExist($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1)));


        $dret_ = $this->ossClient->deleteObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $dret['x-oss-version-id']));
    ...
    public function testObjectSymlink()
    {
        ...
        $this->assertTrue(isset($sym1[OssConst::OSS_HEADER_VERSION_ID]));
        $this->assertTrue(isset($sym2[OssConst::OSS_HEADER_VERSION_ID]));

        $versionId1 = $ret1[OssConst::OSS_HEADER_VERSION_ID];
        $versionId2 = $ret2[OssConst::OSS_HEADER_VERSION_ID];

        $sym_versionId1 = $sym1[OssConst::OSS_HEADER_VERSION_ID];
        $sym_versionId2 = $sym2[OssConst::OSS_HEADER_VERSION_ID];


        $sym_ret = $this->ossClient->getSymlink($this->bucket, $symlink);
        $sym_ret1 = $this->ossClient->getSymlink($this->bucket, $symlink, array(OssConst::OSS_VERSION_ID => $sym_versionId1));
        $sym_ret2 = $this->ossClient->getSymlink($this->bucket, $symlink, array(OssConst::OSS_VERSION_ID => $sym_versionId2));
        ...
        $res1 = $this->ossClient->getObject($this->bucket, $symlink, array(OssConst::OSS_VERSION_ID => $sym_versionId1));
        $res2 = $this->ossClient->getObject($this->bucket, $symlink, array(OssConst::OSS_VERSION_ID => $sym_versionId2));
        $this->assertEquals($content1, $res1);
        $this->assertEquals($content2, $res2);
        $this->assertEquals($content2, $res);
    }
    ...
    public function testObjectCopy()
    {
        ...
        $cret1 = $this->ossClient->copyObject($this->bucket, $object, $to_bucket, $to_object1, array(OssConst::OSS_VERSION_ID => $versionId1));
        $cret2 = $this->ossClient->copyObject($this->bucket, $object, $to_bucket, $to_object2, array(OssConst::OSS_VERSION_ID => $versionId2));
    ...
    public function testObjectRestore()
    {
        ...
        $content1 = 'hello';
        $content2 = 'hello world';
        $ret1 = $this->ossClient->putObject($this->bucket, $object, $content1, array(OssConst::OSS_HEADERS => array('x-oss-storage-class' => 'Archive')));
        $ret2 = $this->ossClient->putObject($this->bucket, $object, $content2);
        ...
        try{
            $this->ossClient->getObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        ...
        $result = $this->ossClient->restoreObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        common::waitMetaSync();
        $this->assertEquals('202', $result['info']['http_code']);

        try{
            $this->ossClient->restoreObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $versionId1));
        }catch(OssException $e){
    ...
    public function testObjectMultiPart()
    {
        ...
        $copyId = 1;
        $eTag = $this->ossClient->uploadPartCopy($this->bucket, $object_src, $this->bucket, $object1, $copyId, $upload_id, array(OssConst::OSS_VERSION_ID => $versionId1));
        $upload_parts1[] = array(
            'PartNumber' => $copyId,
            'ETag' => $eTag,
        );
        ...
        $copyId = 1;
        $eTag = $this->ossClient->uploadPartCopy($this->bucket, $object_src, $this->bucket, $object2, $copyId, $upload_id, array(OssConst::OSS_VERSION_ID => $versionId2));
        $upload_parts2[] = array(
            'PartNumber' => $copyId,
            'ETag' => $eTag,
        );
    ...
    public function testObjectMisc()
    {
        ...
        $res = $this->ossClient->getObject($this->bucket, $object);
        $sres1 = $this->ossClient->getObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $sret1['x-oss-version-id']));
        $sres2 = $this->ossClient->getObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $sret2['x-oss-version-id']));
        $bres1 = $this->ossClient->getObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $bret1['x-oss-version-id']));
        $bres2 = $this->ossClient->getObject($this->bucket, $object, array(OssConst::OSS_VERSION_ID => $bret2['x-oss-version-id']));
    ...
    public function tearDown():void
    {
        ...
        foreach ($versions as $obj) {
            $options = array(
                OssConst::OSS_VERSION_ID => $obj->getVersionId(),
            );
        ...
        foreach ($deleteMarkers as $del) {
            $options = array(
                OssConst::OSS_VERSION_ID => $del->getVersionId(),
            );
