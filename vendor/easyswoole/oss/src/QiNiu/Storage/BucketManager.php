...
namespace EasySwoole\Oss\QiNiu\Storage;
...
use EasySwoole\Oss\QiNiu\Http\Error;
...
final class BucketManager
{
    ...
    {
        $path = '/stat/' . Util::entry($bucket, $key);
    ...
    {
        $path = '/delete/' . Util::entry($bucket, $key);
    ...
    {
        $from = Util::entry($from_bucket, $from_key);
        $to = Util::entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        if ($force === true) {
            $path .= '/force/true';
        }
    ...
    {
        $resource = Util::entry($bucket, $key);
        $encode_mime = Util::base64_urlSafeEncode($mime);
     ...
     *                         失败时：  [
     *                                          null,
     *                                         EasySwoole\Oss\QiNiu/Http/Error
     *                                  ]
    ...
    public function fetch($url, $bucket, $key = null)
    {
        ...
        $resource = Util::base64_urlSafeEncode($url);
        $to = Util::entry($bucket, $key);
    ...
    private function post($url, $body)
    {
        ...
        $r = ($ret->getBody() === null) ? array() : $ret->json();
        return array($r, null);
    }
    ...
    private function postV2($url, $body)
    {
        ...
        $r = ($ret->getBody() === null) ? array() : $ret->jsonSerialize();
        return array($r, null);
    }
    ...
    public static function buildBatchDeleteAfterDays($bucket, $key_day_pairs)
    {
        ...
        foreach ($key_day_pairs as $key => $day) {
            array_push($data, '/deleteAfterDays/' . Util::entry($bucket, $key) . '/' . $day);
        }
        return $data;
    }
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . Util::entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket, $force)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = Util::entry($source_bucket, $from_key);
            $to = Util::entry($target_bucket, $to_key);
